...
<xsl:import href="code-tables/tei-labels.xsl"/>
...
<xsl:template match="xs:element[@name] | xs:element[@ref]">
   <xsl:choose>
      ...
      <!-- create an XForms group label with a label for each element -->
         <xf:group>
           <xsl:attribute name="ref" select="concat('/', @name)"/>
           <xf:label class="group-label"><xsl:value-of select=" @name"/></xf:label>
           <xsl:apply-templates/>
            <xf:submit submission="save">
                <xf:label>Save <xsl:value-of select="substring-before(@name, 'Document')"/></xf:label>
            </xf:submit>
        </xf:group>
      </xsl:when>
      <xsl:when test="@name">
...
<xsl:template name="leaf-node">
   ...
   <xsl:choose>
      <xsl:when test="ends-with($leaf-name, 'title')">
         <xf:input>
            ...
            <xf:label><xsl:value-of select="$screen-label"/>: </xf:label>
         </xf:input>
      </xsl:when>
      <xsl:when test="ends-with($leaf-name, 'author')">
        <xf:select1>
       ...
       <xsl:when test="ends-with($leaf-name, 'editor')">
         <xf:input>
             <xsl:attribute name="ref" select="$leaf-name"/>
            <xf:label><xsl:value-of select="$screen-label"/>: </xf:label>
          </xf:input>
       </xsl:when>
       <xsl:when test="ends-with($leaf-name, 'publisher')">
          <xf:input>
             <xsl:attribute name="ref" select="$leaf-name"/>
             <xf:label><xsl:value-of select="$screen-label"/>: </xf:label>
          </xf:input>
       ...
       <xsl:when test="ends-with($leaf-name, 'Desc')">
       </xsl:when>
       <xsl:when test="ends-with($leaf-name, 'Stmt')">
       </xsl:when> 
       <xsl:when test="ends-with($leaf-name, 'TEI')">
       </xsl:when>
       <xsl:when test="ends-with($leaf-name, 'teiHeader')">
       </xsl:when>       
       ...
       <xsl:otherwise>
          <b><font color="red">Error - type for <xsl:value-of select="$leaf-name"/> not found.</font></b>
       </xsl:otherwise>
       </xsl:choose>
   <xsl:text>
      </xsl:text>

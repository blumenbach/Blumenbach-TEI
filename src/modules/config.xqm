...
module namespace config="http://exist-db.org/xquery/apps/blume/config";
...
declare variable $config:exist-context := "/exist/apps";
declare variable $config:orbeon-fr := "http://" || request:get-server-name() || ":" || request:get-server-port() || "/orbeon/fr/";
declare variable $config:orbeon-bb := $config:orbeon-fr || "blumenbach";
...
declare variable $config:orbeon-sammlung-collection := "/db/orbeon/fr/blumenbach-sammlung/main/data";
declare variable $config:orbeon-publish := "/db/apps/blumenbach/data/publish";
declare variable $config:restxq := "/restxq";
declare variable $config:theme-root := theme:get-default();
declare variable $config:resources := $config:exist-context || "/blumenbach/resources";
declare variable $config:themes := $config:app-root || "/pages";
declare variable $config:theme-config := $config:themes || "/configuration.xml";
declare variable $config:cat-config := "/db/apps/blumenbach/data/taxons";
declare variable $config:admin-config := $config:themes || "/admin-functions.xml";
declare variable $config:query-config := $config:themes || "/queries.xml";
...
declare variable $config:xqd-data := $config:app-root || "/xqdocs";
declare variable $config:config := doc(concat($config:app-root, "/configuration.xml"));
declare variable $config:wiki-root := 
    let $root := $config:config/configuration/docroot/string()
    return
      concat($config:app-root, "/", $root)
;
declare variable $config:system-user := $config:config/configuration/system-user/string();
declare variable $config:system-password := $config:config/configuration/system-password/string();
...
declare function config:xqdoc-styles($node as node(), $model as map(*)) as element()* {
        <link rel="shortcut icon" type="image/png" href="{$config:resources}/images/schädel.png"/>,
        <link rel="stylesheet" type="text/css" href="{$config:resources}/css/font/font-awesome.css"/>,
        <link rel="stylesheet" type="text/css" href="{$config:resources}/css/fundocs.css"/>
};

declare function config:app-scripts($node as node(), $model as map(*)) as element()* {
        <script type="text/javascript" src="{$config:resources}/scripts/jquery/jquery-1.12.4.js"/>,

...
: Module Overview: Defines all the RestXQ endpoints.
:)

(:~ Defines all the RestXQ endpoints.
:)

module namespace restxq="http://exist-db.org/xquery/apps/restxq";
...
function restxq:briefs() {
    ...
    let $html :=
        ...
        return
                if ($brief) then       
                    let $xsl1 := doc("/db/apps/blumenbach/styles/briefs-identity.xsl")
     ...
     return
        <html>
            <head>
                ...
                <link rel="stylesheet" type="text/css" href="/exist/apps/blumenbach/resources/css/br-stylesheet.css"/>
                <link rel="shortcut icon" type="image/png" href="/exist/apps/blumenbach/resources/images/schädel.png"/>            
            </head>
            <body>
            {$html}
            </body>
...
function restxq:werke() {
    ...
    let $html :=
        ...
        return
            if ($work) then       
                let $xsl1 := doc("/db/apps/blumenbach/styles/werke-identity.xsl")
     ...
     return
        <html xmlns="http://www.tei-c.org/ns/1.0">
            <head>
                ...
                <link rel="stylesheet" type="text/css" href="/exist/apps/blumenbach/resources/css/werke-stylesheet.css"/>
                <link rel="shortcut icon" type="image/png" href="/exist/apps/blumenbach/resources/images/schädel.png"/>            
            </head>
            <body>
            {$menu}
...
(:~
 : Return an object dataset as JSON.
 :)
declare
    %rest:GET
    %rest:path("/object/{$ogid}")
    %rest:produces("application/json")
    %output:method("json")
    %output:indent("yes")    
function restxq:get-object-json($ogid as xs:string*) {
    let $objects := collection($config:orbeon-sammlung-collection)
    let $context := doc("/db/orbeon/fr/blumenbach-sammlung/main/static/sammlung-context.xml")
    let $obj :=
        for $object in $objects
        where $object//a2 || $object//a3 = $ogid
        return
                if ($object) then       
                    let $xsl1 := doc("/db/apps/blumenbach/styles/object-json.xsl")
                    let $out := transform:transform($object, $xsl1, ())            
                    return
                    $out
                else ()
    return

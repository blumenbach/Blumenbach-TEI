...
module namespace xqd-app="http://exist-db.org/xquery/xqd-app";
...
declare variable $xqd-app:MD_MODULE_URI := "http://exist-db.org/xquery/markdown";

declare variable $xqd-app:MD_HAS_MODULE :=
    try {
        inspect:inspect-module-uri(xs:anyURI($xqd-app:MD_MODULE_URI))
    } catch * {
        ()
    };
    
declare variable $xqd-app:MD_CONFIG := map {
...
declare function xqd-app:check-dba-user($node as node(), $model as map(*)) {
...
declare function xqd-app:check-dba-user-and-not-data($node as node(), $model as map(*)) {
    let $data := collection($config:xqd-data)/xqdoc:xqdoc
    let $user := xmldb:get-current-user()
    return
...
declare function xqd-app:check-dba-user-and-data($node as node(), $model as map(*)) {
    let $data := collection($config:xqd-data)/xqdoc:xqdoc
    let $user := xmldb:get-current-user()
    return
...
declare function xqd-app:check-not-data($node as node(), $model as map(*)) {
    let $data := collection($config:xqd-data)/xqdoc:xqdoc
    return
        if (not($data)) then
            $node
...
declare function xqd-app:check-not-dba-user-and-not-data($node as node(), $model as map(*)) {
    let $data := collection($config:xqd-data)/xqdoc:xqdoc
    let $user := xmldb:get-current-user()
    return
...
function xqd-app:action($node as node(), $model as map(*), $action as xs:string, $module as xs:string?, 
    $q as xs:string?, $type as xs:string) {
    switch ($action)
        case "browse" return
            xqd-app:browse($node, $module)
        case "search" return
            xqd-app:search($node, (), $q, $type)
        default return
            ()
};

declare %private function xqd-app:browse($node as node(), $module as xs:string?) {
    let $functions := collection($config:xqd-data)/xqdoc:xqdoc[xqdoc:module/xqdoc:uri = $module]//xqdoc:function
    return
        map { "result" := $functions }
};

declare %private function xqd-app:search($node as node(), $module as xs:string?, 
    $q as xs:string?, $type as xs:string) {
    let $functions :=
        switch( $type )
        case "name" return
            collection($config:xqd-data)/xqdoc:xqdoc//xqdoc:function[ngram:contains(xqdoc:name, $q)]
        case "signature" return
            collection($config:xqd-data)/xqdoc:xqdoc//xqdoc:function[ngram:contains(xqdoc:signature, $q)]
        case "desc" return
            collection($config:xqd-data)/xqdoc:xqdoc//xqdoc:function[ngram:contains(xqdoc:comment/xqdoc:description, $q)]
        default return ()
...
function xqd-app:module($node as node(), $model as map(*), $details as xs:boolean) {
    ...
    order by $order
    return
        xqd-app:print-module($module, $funcsInModule, $details)
};

declare %private function xqd-app:print-module($module as element(xqdoc:xqdoc), $functions as element(xqdoc:function)*, 
    $details as xs:boolean) {
    let $location := $module/xqdoc:control/xqdoc:location/text()
    let $uri := $module/xqdoc:module/xqdoc:uri/text()
    let $extDocs := xqd-app:get-extended-module-doc($module)[1]
    ...
    <div class="module" data-xqdoc="{document-uri(root($module))}">
        ...
        </div>
        {
            if ($details and exists($extDocs)) then
                <div class="extended">
                    <h1>Overview</h1>
                    { xqd-app:parse-markdown($extDocs) }
                </div>
            else
                ()
        }
        <div class="functions">
            ...
            }
            {
                ...
                return
                    xqd-app:print-function($function, false())
            }
        </div>
    </div>
};

declare %private function xqd-app:print-function($function as element(xqdoc:function), $details as xs:boolean) {
    ...
    let $parsed := if (contains($description, '&lt;') or contains($description, '&amp;')) then $description else util:parse("<div>" || replace($description, "\n{2,}", "<br/>") || "</div>")/*/node()
    let $extDocs := xqd-app:get-extended-doc($function)[1]
    return
            ...
            <div class="function-detail">
                <p class="description">{ $parsed }</p>
                {
                    if (exists($extDocs) and not($details)) then
                        ...
                        return
                            <a href="xqdocs-view.html{$query}" class="extended-docs btn btn-primary">
                ...
                <dl class="parameters">          
                    {
                        if($comment/xqdoc:param) then 
                            (
                                ...
                                <dd>
                                {
                                    xqd-app:print-parameters($comment/xqdoc:param)
                                }
                                </dd>
                            )
                        else 
...
declare %private function xqd-app:parse-markdown($path as xs:string) {
    if ($xqd-app:MD_HAS_MODULE) then
        let $expr := 
            'import module namespace md="http://exist-db.org/xquery/markdown";' ||
            'md:parse(util:binary-to-string(util:binary-doc($path)), $xqd-app:MD_CONFIG)'
        return
            util:eval($expr)
    else
...
declare %private function xqd-app:print-parameters($params as element(xqdoc:param)*) {
    <table>
    {
...
declare %private function xqd-app:get-extended-doc($function as element(xqdoc:function)) {
    ...
    let $paths := (
        ...
        $config:wiki-root || "/" || $prefix || "/" || $name || "_" || $arity || ".md",
        (: General file without arity :)
        $config:wiki-root || "/" || $prefix || "/" || $name || ".md"
    )
    for $path in $paths
    return
...
declare %private function xqd-app:get-extended-module-doc($module as element(xqdoc:xqdoc)) {
    let $prefix := $module/xqdoc:module/xqdoc:name
    let $prefix := if ($prefix/text()) then $prefix else "fn"
    let $paths := (
        (: Module description is either "_module.md" or prefix.md :)
        $config:wiki-root || "/" || $prefix || "/_module.md",
        $config:wiki-root || "/" || $prefix || "/" || $prefix || ".md"
    )
    for $path in $paths
    return
        if (util:binary-doc-available($path)) then
...
(: ~
 ...
 : from cloudfront.
 :)
declare function xqd-app:import-ace($node as node(), $model as map(*)) {
...
function xqd-app:showmodules($node as node(), $model as map(*),  $w3c as xs:boolean, $extensions as xs:boolean, $appmodules as xs:boolean) {
    
    for $module in collection($config:xqd-data)//xqdoc:xqdoc
    ...
    order by $uri
    return
    <tr><td><a href="xqdocs-view.html{$query}">{$uri}</a></td><td>{$location}</td></tr> 
(:    
    order by $uri
    return 
        if ( 
...
declare 
    ...
    %templates:default("details", "false")
function xqd-app:view($node as node(), $model as map(*),  $uri as xs:string, $location as xs:string?, $function as xs:string?,
    $arity as xs:int?, $details as xs:boolean) {
    
    let $modules :=  
        if ($location)  then
            collection($config:xqd-data)/xqdoc:xqdoc[xqdoc:module/xqdoc:uri eq $uri][xqdoc:control/xqdoc:location eq $location]
        else
            collection($config:xqd-data)/xqdoc:xqdoc[xqdoc:module/xqdoc:uri eq $uri]
    return
        ...
        return
            if ($function) then
                ...
                return
                    xqd-app:print-function($xqdocfunction, exists($function))
            else
                xqd-app:print-module($module, $module//xqdoc:function, $details)
};

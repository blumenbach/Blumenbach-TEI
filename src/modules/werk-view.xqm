...
(:
: Module Name: werk-view
: Module Version: 
: Date: 
: License: GNU-LGPL
: 
: XQuery Specification : XQuery v3.0
: Module Overview: Functions for single Werk view
:)

(:~ Functions for single Werk view
:)

module namespace werk-view="http://exist-db.org/xquery/werk-view";
                ...
                return
                  <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                            {for $title in $monogr
                            where $title/tei:title[@type="aut"]
                            return 
                                werk-view:convert-meta($title/tei:title[@type="aut"])
                            }    
                            {for $title in $monogr
                            where $title/tei:title[@type="main"]
                            return 
                                werk-view:convert-meta($title/tei:title[@type="main"])
                            }                                
                            {for $title in $monogr
                            where $title/tei:title[@type="sub"]
                            return if (not($analytic/tei:title/text())) then
                                 werk-view:convert-meta($title/tei:title[@type="sub"])
                                 else
                                 ()
                            }
                            {for $title in $monogr
                            where $title/tei:title[@type="ed"]
                            return 
                                werk-view:convert-meta($title/tei:title[@type="ed"])
                            }
        ...
        case element(tei:span)
            ...
            let $span :=
                ...
                <span>{werk-view:convert-meta($node)}&#160;</span>
                    ...
                    <span>&#160;<img style="width: 16px; height: 16px;" src="/exist/apps/blumenbach/resources/images/work.png"/>&#160;</span>                     

...
: License: GNU-LGPL
: 
: Module Overview: xslt form test
:)

(:~
 : Test of xslt xforms processing
 :)
...
return
    if ($form-name)
    then 
        let $form-doc := doc($form-path)/tei:TEI
        ...
        let $eXide-button-form-path :=
            <div xmlns="http://www.w3.org/1999/xhtml" class="source">
            <div class="toolbar">
                    <a class="btn" href="/apps/eXide/index.html?open={$form-path}" target="eXide" data-type="XML"
        ...
        let $xslt-pi := processing-instruction xml-stylesheet {'type="text/xsl" href="/rest/db/apps/xsltforms/xsltforms.xsl"'}

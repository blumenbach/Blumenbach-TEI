...
import module namespace login-helper="http://exist-db.org/xquery/login-helper" at "modules/login-helper.xql";
import module namespace config="http://exist-db.org/xquery/apps/blume/config" at "modules/config.xqm";
...
declare variable $exist:context := '/exist';
...
(: Requests for javascript libraries are resolved to the file system :)
    ...
    return
        if (ends-with($exist:resource, ".epub")) then
        ...
        else if ($exist:resource = "redirect-orbeon.xml") then
            let $resource := request:get-parameter("page", ())
            return
            <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
                <redirect url="{$config:orbeon-fr}{$resource}">
                </redirect>     	
                <cache-control cache="no"/>
            </dispatch>            
        else if ($exist:resource = "export.xml") then
            <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
                <forward url="{$exist:controller}/modules/export.xql">
                    <add-parameter name="action" value="{$type}"/> 
                </forward>     	
                <cache-control cache="no"/>
            </dispatch>
        else if ($exist:resource = "random.xml") then
            let $type := request:get-parameter("type", ())
            let $random := 
               if ($type = "werk") then
                functx:pad-integer-to-length(util:random(1025), 5)
               else 
               functx:pad-integer-to-length(util:random(1665), 4)
            let $dispatch :=
                if ($type = "werk") then
                    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
                        <redirect url="{$exist:context}/{$exist:prefix}/{$exist:controller}/view-work.html?id={$random}"></redirect>
                        <error-handler>
                            <forward url="{$exist:controller}/error-page.html" method="get"/>
                            <forward url="{$exist:controller}/modules/view.xql"/>
                        </error-handler>
                    ...
                    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
                        <redirect url="{$exist:context}/{$exist:prefix}/{$exist:controller}/view-brief.html?id={$random}"></redirect>
                        <error-handler>
                            <forward url="{$exist:controller}/error-page.html" method="get"/>
                            <forward url="{$exist:controller}/modules/view.xql"/>
                        </error-handler>
            ...
            if (request:get-attribute("org.exist.login.user")) then        
                <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
                    ...
                    <view>
                        <redirect url="{$exist:context}/{$exist:prefix}/{$exist:controller}/admin/view-form-data.html">                     
                        </redirect>
                    </view>
                </dispatch>
            else
            ...
            if (request:get-attribute("org.exist.login.user")) then        
                <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
                    <forward url="{$exist:controller}/modules/publish.xqm">
                     ...
                     <view>
                        <redirect url="{$exist:context}/{$exist:prefix}/{$exist:controller}/admin/view-xform-tei.html">                    
                        </redirect>
                    </view>
                </dispatch>
            else

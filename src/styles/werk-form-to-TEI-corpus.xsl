...
<xsl:stylesheet xmlns:saxon="http://saxon.sf.net/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:so="http://standoff.proposal" extension-element-prefixes="saxon" version="3.0">
    <xsl:param name="output.media-type"/>
    <xsl:output method="xml" indent="yes" media-type="{$output.media-type}"/>
    <xsl:template match="/">
        ...
        <teiCorpus xmlns="http://www.tei-c.org/ns/1.0" xml:lang="de-DE">
            ...
            <xsl:for-each select="//form">
                ...
                <xsl:variable name="filename" select="concat('jfb_werke_corpus_', $number, '.xml')"/>
                <TEI xml:lang="de-DE">
                    <teiHeader>
                        <fileDesc>
                            <titleStmt>
                                ...
                                <biblStruct>
                                    ...
                                    <monogr>
                                        <xsl:if test="./sourceDesc-analytic/monograph-title-level.j/text()">
                                            <title level="j">
                                                <xsl:value-of select="./sourceDesc-analytic/analytic-title-level.j"/>
                                            </title>
                                        </xsl:if>
                    ...
                    <so:stdf>
                        <so:annotations n="{$number}">
                            <xsl:for-each select="./stdf-annotations/so-annotations">
                                ...
                                <spanGrp>
                                    <xsl:value-of select="$spangrp" disable-output-escaping="yes"/>
                                </spanGrp>
                            </xsl:for-each>
                        </so:annotations>
                    </so:stdf>

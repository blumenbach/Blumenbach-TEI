            ...
            <xsl:result-document href="{$filename}">
                ...
                <TEI xmlns="http://www.tei-c.org/ns/1.0" xml:lang="de-DE" xmlns:so="http://standoff.proposal">
                    <teiHeader>
                               ...
                               <xsl:otherwise>
                                ...
                                <monogr>                                   
                                   <xsl:if test="./title[@level='j']"><title level="j"><xsl:value-of select="./title[@level='j']"/></title></xsl:if>
                                    <xsl:if test="./title[@level='ma']"><title level="m"><xsl:value-of select="./title[@level='ma']"/></title></xsl:if>
                                   <xsl:if test="./title[@type='sub']">
                                       <xsl:for-each select="title[@type='sub']">
                                            <title level="m" type="sub"><xsl:value-of select=".[@type='sub']"/></title>
                                       </xsl:for-each>
                        ...
                        <revisionDesc>
                            <change>Erstellungsdatum: <xsl:value-of select="current-dateTime()"/></change>
                        </revisionDesc>                        
                    </teiHeader>
                    <so:stdf>
		              <so:annotations n="{format-number($count, '00000')}">
		                  ...
		                  <xsl:attribute name="xml:id">
		                      <xsl:value-of select="concat(concat(name(),'_',generate-id()),'_', format-number($count, '00000'))"></xsl:value-of>
		                  </xsl:attribute>    

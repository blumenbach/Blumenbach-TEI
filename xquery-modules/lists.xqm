...
module namespace list="http://exist-db.org/lists/";
...
function list:briefs-all($node as node(), $model as map(*)) {
   ...
   let $titleStmt := $brief/ancestor-or-self::tei:teiHeader/tei:fileDesc/tei:titleStmt
   let $creation := $brief/ancestor-or-self::tei:teiHeader/tei:profileDesc/tei:creation
   ...
   let $titel :=     
        ...
        return
            <a xmlns="http://www.w3.org/1999/xhtml" href="{$href}">{$title}</a>
   let $dsort := for $date in $creation/tei:origDate/tei:note/tei:listEvent/tei:event
                return data($date/@sortKey)
